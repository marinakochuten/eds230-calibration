...
source(here("compute_highflowmetrics_all.R"))
...
#### monthly flow
```{r}
# we could also calculate mse for june flow
# get monthly flow
...
tmp = mse_flow_all %>% subset(month==6) 
...
```{r}
p1=ggplot(subset(msagel_acc_month, month==6), aes(wy, str, group=wy)) +
  geom_boxplot(position="dodge") +
  geom_point(aes(wy, obs), col="red") +
  labs(y = "Streamflow", x = "Date")

p1 + geom_point(data=subset(mse_flow, month==6), aes(wy, str_mse), col="blue") 
```
...
mean_june_obs = mean(subset(msagel_month_avg, month==6)$obs)
```

```{r}
p2 = ggplot(subset(msagel_month_avg, month==6), aes(x=accept, y=str))+geom_boxplot()+geom_hline(yintercept=mean_june_obs, col="red")+geom_boxplot(aes(x=othercal, y=str))  + labs(y = "Mean Streamflow\n", x = "Parameter set") +
